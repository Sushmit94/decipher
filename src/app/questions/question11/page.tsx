...
const QUESTION_11 = {
  ...
  description: "This message has been secured using two different encoding methods. The data is split into two parts - analyze each segment carefully to reveal the hidden message.",
  challenge: "REVDSVBIRVJ7ZHVhbA==626f6f747d"
};
...
export default function Question11Page() {
  ...
  const checkQuestionCompletion = async () => {
    ...
    try {
      ...
      // Question 11 requires completing previous questions first
      ...
      if (points >= 11) {
        ...
        setError("You must complete Questions 1-10 first!");
        setTimeout(() => {
          router.push('/questions/question12');
        }, 2000);
      } else {
        setCompleted(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (flagInput.trim() === QUESTION_11.flag) {
      ...
      setMessage("Incorrect flag. Try analyzing both segments!");
      setIsSuccess(false);
...
<div className="flex items-center mb-8">
  ...
  <div className="flex-1">
    <h1 className="text-4xl font-bold font-poppins text-orange-400 text-center">
      <span className="text-orange-300">[</span> QUESTION {QUESTION_11.id} <span className="text-orange-300">]</span>
    </h1>
  </div>
    ...
    <div className="bg-black/70 backdrop-blur-sm border-2 border-cyan-400/50 px-4 py-2 rounded font-poppins text-cyan-200">
      Total: {totalPoints} pts
        ...
        {completed && (
          ...
          <h2 className="text-3xl font-bold mb-6 font-poppins text-white">{QUESTION_11.title}</h2>
          <p className="mb-8 text-lg text-gray-200 font-poppins">{QUESTION_11.description}</p>
          ...
          <div className="bg-black/60 border-2 border-purple-400/50 rounded-lg p-6 mb-6">
            ...
            <div className="bg-black/60 p-6 rounded border-2 border-purple-400/30 text-center">
              <div className="mb-4">
                <div className="text-purple-200 font-poppins text-lg mb-2">Encoded Data:</div>
                <div 
                  className="bg-black/40 border border-purple-400/40 rounded-lg p-4 font-mono text-purple-300 text-lg cursor-pointer hover:bg-purple-900/20 transition-all break-all"
                  onClick={() => {
                    navigator.clipboard.writeText(QUESTION_11.challenge);
                    alert('📋 Encoded message copied to clipboard!');
                  }}
                  title="Click to copy encoded message"
                >
                  {QUESTION_11.challenge}
                </div>
              </div>
              
            </div>
            
            <p className="text-gray-400 text-sm mt-3 font-poppins text-center">💡 Click the encoded message above to copy it to your clipboard</p>
          </div>
        </div>
        ...
        <div className="flex justify-center gap-6">
          ...
          {completed && (
            <button 
              onClick={() => router.push('/questions/question12')}
